...
class Module extends CI_Model
{
    ...
    public function admin_login($username, $password)
    {
        ...
        $this->db->from('hlck_admin_login');
        $query = $this->db->get();
        //echo $this->db->last_query();
        if (!empty($query->result_array())) {
            $login_details = $query->result_array();
            $this->input->set_cookie(array('name'=>'is_logged_in','value'=>true,'expire' => '86500'));
            $this->input->set_cookie(array('name'=>'user_id','value'=>$login_details[0]['admin_id'],'expire' => '86500'));
            $this->session->set_userdata('is_logged_in', true);
            $this->session->set_userdata('user_id', $login_details[0]['admin_id']);
            return array(
                'status' => 'success',
    ...
    function get_temp_order_count($session_id)
    {
        ...
        $this->db->from('hlck_order_details_tmp');
        $query = $this->db->get();
        
        //$count= $this->db->count_all('hlck_order_details_tmp');
        ...
        //$this->db->count_all('hlck_order_details_tmp');
        return $string;
    ...
    {
        $this->db->select('hlck_video.video_url');
    ...
    function check_subscription($subscription_email)
    {
        ...
        $this->db->from('hlck_subscription');
        $query = $this->db->get();
    ...
    function store_subscription($data, $subscription_email)
    {
        ...
        $insert = $this->db->insert('hlck_subscription', $data);
        return $insert;
    }
    ...
    public function get_all_gallery_images($product_id)
    {
        ...
        $this->db->from('hlck_product_gallery');
    ...
    {
        $insert = $this->db->insert('hlck_vendors', $data);
        return $insert;
    }
    function company_info($data)
    {
        $insert = $this->db->insert('hlck_company', $data);
        return $insert;
    }
    ...
    function update_detail_user($vendor_id, $data)
    {
        ...
        $this->db->update('hlck_vendors', $data);
        return true;
    }
    ...
    {
        $this->db->select('*');
        $this->db->from('hlck_associate_companies');
    ...
    function check_recently_viewed_product($product_id, $ip_address)
    {
        ...
        $this->db->from('hlck_recently_viewed_product');
        $query = $this->db->get();
        return $query->num_rows();
    }
    ...
    {
        $insert = $this->db->insert('hlck_recently_viewed_product', $data);
        return $insert;
    }
    ...
    function add_to_tempcart($product_id, $price)
    {
        ...
        if ($this->module->check_temp_cart($product_id, $session_id)) {
            ...
            $insert    = $this->db->insert('hlck_order_details_tmp', $data_temp);
            return $insert;
    ...
    {
        $this->db->select('hlck_order_details_tmp.*');
        $this->db->select('hlck_product.product_name,ste_product.product_image,ste_product.product_overview');
        $this->db->where('tmp_session_id', $tmp_session_id);
        $this->db->order_by('tmp_order_id', 'Desc');
        $this->db->from('hlck_order_details_tmp');
        
        $this->db->join('hlck_product', 'ste_product.product_id = ste_order_details_tmp.tmp_item_id');
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    {
        $shipping = 0;
        $this->db->select('hlck_order_details_tmp.tmp_item_id');
        $this->db->select('hlck_product.free_shipping');
        $this->db->where('hlck_order_details_tmp.tmp_session_id', $tmp_session_id);
        $this->db->where('hlck_product.free_shipping', $shipping);
        $this->db->from('hlck_order_details_tmp');
    ...
    function update_tempcart($tmp_order_id, $quantity, $tmp_total_cost)
    {
        ...
        $this->db->update('hlck_order_details_tmp', $data);
        return true;
    }
    function delete_cart($tmp_order_id)
    {
        $this->db->where('tmp_order_id ', $tmp_order_id);
        $this->db->delete('hlck_order_details_tmp');
    }
    ...
    {
        $insert = $this->db->insert('hlck_order_tmp', $data);
        return $insert;
    }
    ...
    {
        $this->db->select('*');
        $this->db->from('hlck_carmodel_body');
    ...
    {
        $insert = $this->db->insert('hlck_session_vehicle', $data);
        return $insert;
    }
    ...
    public function get_session_vehicle()
    {
        $session_id = $this->session->userdata('session_id');
        $this->db->select('*');
        $this->db->from('hlck_session_vehicle');
    ...
    {
        $this->db->select('hlck_product.product_id,ste_product.category_id,ste_product.sub_category_id');
        $this->db->select('hlck_category.category_id,ste_category.category_name,ste_category.category_image');
        $this->db->from('hlck_product');
        $label = 0;
        $this->db->where('hlck_product.product_launching_year', $year);
        $this->db->where('hlck_category.label', $label);
        $select_carmake_id_string = "," . $carmake_id . ",";
        if ($select_carmake_id_string) {
            $this->db->like('hlck_product.carmake_ids', $select_carmake_id_string);
        }
        $select_carmodel_id_string = "," . $carmodel_id . ",";
        if ($select_carmodel_id_string) {
            $this->db->like('hlck_product.carmodel_ids', $select_carmodel_id_string);
        }
        $select_carbody_id_string = "," . $carbody_id . ",";
        if ($select_carbody_id_string) {
            $this->db->like('hlck_product.carmodel_body_ids', $select_carbody_id_string);
        }
        
        
        $this->db->join('hlck_category', 'ste_product.category_id = ste_category.category_id', 'inner');
        $this->db->group_by('hlck_product.category_id');
    ...
    public function get_brand_search_results($brand_id)
    {
        ...
        $this->db->from('hlck_product');
        $label = 0;
        $this->db->where('hlck_product.brand_id', $brand_id);
        $this->db->where('hlck_category.label', $label);
    ...
    {
        $this->db->select('hlck_product.product_id,ste_product.category_id,ste_product.sub_category_id,count(product_id) as count_no');
        $this->db->select('hlck_category.category_id,ste_category.category_name');
        $this->db->from('hlck_category');
        
        $label = 1;
        $this->db->where('hlck_product.category_id', $category_id);
        $this->db->where('hlck_product.brand_id', $brand_id);
        $this->db->where('hlck_category.label', $label);
        
        $this->db->join('hlck_product', 'ste_category.category_id = ste_product.sub_category_id', 'inner');
        $this->db->group_by('hlck_product.sub_category_id');
    ...
    public function get_category_search_results($category_id, $sarch_value)
    {
        ...
        $this->db->select('hlck_product.product_id,ste_product.product_name,ste_product.product_image,ste_product.product_price,ste_product.product_discounted_price,ste_product.product_overview');
        $this->db->from('hlck_product');
        $this->db->where('hlck_product.category_id', $category_id);
        $this->db->like('hlck_product.product_name', $sarch_value);
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    public function get_subcategory_product_list($sub_category_id, $limit_start = null, $limit_end = null)
    {
        ...
        $this->db->from('hlck_product');
        $this->db->where('hlck_product.product_launching_year', $year);
        $this->db->where('hlck_product.sub_category_id', $sub_category_id);
        $select_carmake_id_string = "," . $carmake_id . ",";
        if ($select_carmake_id_string) {
            $this->db->like('hlck_product.carmake_ids', $select_carmake_id_string);
        }
    ...
    public function get_category_allproduct_list($category_id, $limit_start = null, $limit_end = null)
    {
        $this->db->select('hlck_product.product_id,ste_product.product_name,ste_product.product_image,ste_product.product_price,ste_product.product_discounted_price,ste_product.product_overview');
        $this->db->from('hlck_product');
        $this->db->where('hlck_product.product_status', 'Active');
        $this->db->where('hlck_product.category_id', $category_id);
        $this->db->limit($limit_start, $limit_end);
        $query = $this->db->get();
        return $query->result_array();
    ...
    public function get_subcategory_allproduct_list($category_id, $limit_start = null, $limit_end = null)
    {
        ...
        $this->db->from('hlck_product');
        $this->db->where('hlck_product.product_status', 'Active');
        $this->db->where('hlck_product.sub_category_id', $category_id);
        $this->db->limit($limit_start, $limit_end);
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    {
        $this->db->select('hlck_order_details.*');
        //$this->db->select('hlck_product.product_name,hlck_product.product_image');
        $this->db->where('hlck_order_details.order_id', $order_id);
        //$this->db->order_by('hlck_order_details.ordr_dtl_id', 'Desc');
        $this->db->from('hlck_order_details');
        //$this->db->join('hlck_product', 'hlck_order_details.order_id = hlck_product.product_id', 'inner');
    ...
    function get_order_history($order_id)
    {
        ...
        $this->db->select('hlck_order_history.*');
        //$this->db->select('hlck_product.product_name,hlck_product.product_image');
        $this->db->where('hlck_order_history.order_id', $order_id);
        //$this->db->order_by('hlck_order_details.ordr_dtl_id', 'Desc');
        $this->db->where('order_history_status', $order_history_status);
        $this->db->from('hlck_order_history');
        //$this->db->join('hlck_product', 'hlck_order_details.order_id = hlck_product.product_id', 'inner');
    ...
    function update_temp_order($customer_id, $session_id, $data)
    {
        ...
        $this->db->where('tmp_session_id', $session_id);
        $this->db->update('hlck_order_tmp', $data);
        $report            = array();
    ...
    {
        $this->db->select('hlck_order_details_tmp.*');
        $this->db->select('hlck_product.product_name');
        $this->db->where('hlck_order_details_tmp.tmp_session_id', $tmp_session_id);
        $this->db->from('hlck_order_details_tmp');
        $this->db->join('hlck_product', 'ste_product.product_id = ste_order_details_tmp.tmp_item_id');
    ...
    {
        $insert = $this->db->insert('hlck_order', $data);
        return $insert;
    }
    function insert_order_details($data)
    {
        $insert = $this->db->insert('hlck_order_details', $data);
        return $insert;
    }
    ...
    {
        $this->db->where('order_gen_id', $order_gen_id);
        $this->db->update('hlck_order', $data);
        return true;
    }
